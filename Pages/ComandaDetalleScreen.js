...
const ComandaDetalleScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadConfiguracion = async () => {
      try {
        ...
        setMinutosEntregaAuto(minutosEntrega);
      } catch (error) {
    ...
    return configuracionService.subscribeCambio(async () => {
      try {
        const [config, minutosEntrega] = await Promise.all([
  ...
  const refrescarComandas = useCallback(async () => {
    ...
    try {
      ...
      const mesaEstado = mesaEstadoEfectivo;
      ...
      if (comandasMesa.length === 0 && mesaIdRef) {
      ...
      if (comandasMesa.length === 0 && mesaIdRef
        && ['pedido', 'preparado', 'entregado', 'pagado', 'pagando', 'reservado'].includes(mesaEstado)) {
        try {
          const resPag = await axios.get(`${comandaBase}/mesa/${mesaIdRef}/pagadas`, { timeout: 10000 });
          if (resPag.data?.success && Array.isArray(resPag.data.comandas) && resPag.data.comandas.length > 0) {
            comandasMesa = acotarComandasAlCicloActual(
              aplicarPedidoSinVaciar(resPag.data.comandas, resPag.data.pedidoId || pedidoIdCiclo)
            );
          }
        } catch (e) {
      ...
      const hayCicloCerrado = comandasMesa.some((c) =>
        c.IsActive === false
        || !!c.tiempoPagado
        || ['pagado', 'completado'].includes(String(c.status || '').toLowerCase())
      );
      let comandasFinales =
        mesaEstado === 'pagado' || mesaEstado === 'pagando' || mesaEstado === 'pendiente_aprobar' || mesaEstado === 'reservado' || hayCicloCerrado
      ...
      if (comandasFinales.length === 0) {
        ...
        if (mesaSigueEnServicio && prev.length > 0) {
          const prevCerradas = prev.some((c) => c.IsActive === false || !!c.tiempoPagado);
          comandasFinales = prevCerradas ? prev : filtrarComandasActivas(prev);
        }
      }
  ...
  const cocinaPendienteDetalle = todosLosPlatos.some((p) => {
  ...
  // Con espera: el mozo puede adelantar la entrega. Al instante (0) no hace falta el botón.
  const puedeEntregar = minutosEntregaAuto > 0 && todosLosPlatos.some((p) =>
    String(p.estado || '').toLowerCase() === 'salio' && !p.anulado && !p.eliminado
  );
  ...
  const handleAgregarPlato = (plato, cantidadPlatos = 1) => {
    ...
    if (!platoRequiereModalAlSumar(plato) && platoRequiereModalOp(plato)) {
      ...
      if (ultima) {
        const next = platosEditados.map((p) =>
          p === ultima ? { ...p, cantidad: Math.min(99, (p.cantidad || 1) + n) } : p
        );
  ...
  const handleDecrementPlatoFromBuscador = (plato) => {
    const ultima = ultimaLineaDelPlato(platosEditados, plato, tipoServicioMenuEdicion(), { exacto: true });
    if (!ultima) return;
    if ((ultima.cantidad || 1) > 1) {
      const next = platosEditados.map((p) =>
        p === ultima ? { ...p, cantidad: (p.cantidad || 1) - 1 } : p
      );
      platosEditadosRef.current = next;
      setPlatosEditados(next);
      return;
    }
    if (platosEditados.length <= 1) return;
    const next = platosEditados.filter((p) => p !== ultima);
    platosEditadosRef.current = next;
    setPlatosEditados(next);
  };
  ...
  const handleConfirmarComplementosEdicion = ({ complementosSeleccionados, notaEspecial, numeroSerie, _precioUnitario, _extraComplementos, _cantidadPlatos, _partes }) => {
    if (editandoInstanceId) {
      const linea = platosEditadosRef.current.find(
      ...
      if (linea) {
        ...
        const cant0 = Math.max(1, Number(parte.cantidad) || 1);
        ...
        const next = platosEditadosRef.current.filter((p) => {
          const id = String(p.instanceId || p._id);
          return !(idsGrupo.includes(id) && id !== idKeep);
        }).map((p) => {
          if ((p.instanceId || p._id) !== editandoInstanceId) return p;
          return {
            ...p,
            cantidad: cant0,
            complementosSeleccionados: comps,
        ...
        platosEditadosRef.current = next;
        setPlatosEditados(next);
        partes.slice(1).forEach((pExtra) => {
          const calcE = calcularPrecioUnitarioConComplementos(
            linea.precio || 0,
            pExtra.complementos,
            { afectanPrecio: afectan }
          );
          agregarPlatoSinComplementos(
            ...
            pExtra.complementos,
            notaEspecial,
            calcE.precioUnitario,
            calcE.extraComplementos,
            pExtra.cantidad,
            linea.tipoServicio,
            {
              nombreCocinaPedido: pExtra.nombreCocinaPedido,
              variantePlato: pExtra.variantePlato,
              ...(serie ? { numeroSerie: serie } : {}),
            }
          );
        });
                    ...
                    return (
                      ...
                      >
                        ...
                        <View style={styles.platoEditActions}>
                          {platoEditableEnOrdenes(plato, platos) && (
                            <TouchableOpacity
        ...
        cantidadLinea={
          Math.max(
            ...
            Number(
              ...
              || (editandoInstanceId
                ? platosEditados.find((p) => (p.instanceId || p._id) === editandoInstanceId)?.cantidad
